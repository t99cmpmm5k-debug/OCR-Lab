...
const els={
 ...
 errorCard:$("errorCard"),error:$("errorOutput"),debugCard:$("debugCard"),debug:$("debugGrid"),toggle:$("toggleDebugButton"),warningsCard:$("warningsCard"),warningsList:$("warningsList"),warningsBadge:$("warningsBadge")
};
const labels={
 source:["Fuente",""],title:["Título",""],location:["Lugar",""],activity:["Actividad",""],date:["Fecha",""],time:["Hora",""],
...
function reset(){["metricsCard","jsonCard","resultCard","errorCard","debugCard","warningsCard"].forEach(k=>els[k].hidden=true);els.metrics.innerHTML=els.debug.innerHTML="";els.json.textContent=els.ocr.textContent=els.error.textContent="";setProgress(0,1)}
...
function renderMerged(merged){
 ...
 if(warnings.length){els.warningsBadge.textContent=`${warnings.length} aviso${warnings.length===1?"":"s"}`;els.warningsList.innerHTML=warnings.map(w=>`<div class="warning-item">${w}</div>`).join("");els.warningsCard.hidden=false;}
}
function renderDebug(results){
 els.debug.innerHTML=results.map((r,i)=>{
  ...
  return `<article class="debug-item"><h3>Captura ${i+1}</h3><div class="debug-meta">${r.file} · confianza OCR ${r.ocr_confidence}% · ${r.parsed.found} campos</div><div class="debug-fields">${found||'<span class="debug-chip">Sin campos detectados</span>'}</div></article>`
 }).join("");els.debugCard.hidden=false;
}
async function run(files){
 reset();
 if(!window.Tesseract?.recognize)throw new Error("Tesseract.js no se ha cargado.");
 if(!window.GarminParser?.parse||!window.GarminParser?.merge)throw new Error("El parser Garmin V2.5 no se ha cargado.");
