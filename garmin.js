(function(root,factory){
  ...
  function detectTitle(lines){
    const blocked=/^(resumen|estadisticas|vueltas|graficos|equipo|anadir notas|distancia|ritmo medio|tiempo total|calorias totales)$/i;
  ...
  function parse(text){
    const raw=cleanText(text), normalized=normalize(raw), lines=linesOf(raw);
    ...
    const clocks=[...raw.matchAll(/\b([0-2]?[0-9]):([0-5][0-9])\b/g)].map(m=>({value:`${Number(m[1])}:${m[2]}`,source:m[0]})).filter(x=>!avgPace||x.value!==avgPace);
    const title=detectTitle(lines);let location=null,activity=null;
    if(title){const p=title.split(/\s+-\s+/).map(s=>s.trim()).filter(Boolean);if(p.length>=2){location=p[0];activity=p.slice(1).join(" - ")}else activity=title}
    const fields={
      source:field(/garmin|garmin connect|resumen\s+estadisticas\s+vueltas/i.test(normalized)?"Garmin":"Garmin probable","Diseño/texto detectado",.75),
    ...
    return{parser:"garmin-v2.4.0",found:Object.values(fields).filter(x=>x.value!=null).length,data,fields,raw_text:raw};
  }
  function merge(results){
    ...
    const allKeys=["source","title","location","activity","date","time","distance_km","avg_heart_rate_bpm","max_heart_rate_bpm","avg_pace_min_km","total_time","calories_kcal","cadence_spm","temperature_c","elevation_gain_m"];
    ...
    return{parser:"garmin-v2.4.0-merge",found:Object.values(data).filter(v=>v!=null).length,data,fields:mergedFields};
  }
